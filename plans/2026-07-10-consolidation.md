  # Plan: Post-Review Consolidation of Sonnet Rebuild
...
- [x] **Task 7: Fix `BarcodeScannerBottomSheet` to consume Flow-based ScannerProcessor**
...
- [x] **Task 8: Update screen composables for BarcodeScannerBottomSheet signature change**
...
- [x] **Task 9: Write unit tests for critical business logic**
...
- **Round 7:** Task 7 approved (2026-07-10). `BarcodeScannerBottomSheet` signature matches plan spec exactly (5 params: `mode`, `isOpen`, `onDismiss`, `onResult`, `processor`). Zero imports of `GroceryRepository`, `AudioFeedback`, or `OpenFoodFactsClient`. Flow subscriptions via two `LaunchedEffect(Unit)` blocks collecting `scanResultFlow` and `openFoodFactsResultFlow`. `isProcessing` spinner preserved — set/reset on both `processScan` and `createNewProduct` paths, `CircularProgressIndicator` rendered in both `ScanningContent` and `CaptureRequiredContent`. Photo capture fully preserved: `capturePhoto()` with `FileProvider`/`ImageCapture`, `cleanupScannerPhotos()` on dismiss. Barcode detection guarded by `!isProcessing` check. `mode.householdId` used for `createNewProduct`. Note: `capturePhoto.onError` callback lacks Logger — not in Task 7 scope, recommend addressing in a future pass.
- **Round 8:** Task 8 approved (2026-07-10). Both `InventoryScreen` and `ShoppingScreen` receive `scannerProcessor: ScannerProcessor` as a parameter — zero imports of `ScannerViewModel`, `GroceryRepository`, `AudioFeedback`, or `OpenFoodFactsClient`. `BarcodeScannerBottomSheet` called with the full new signature (`mode`, `isOpen`, `onDismiss`, `onResult`, `processor`) in both screens. `GroceryApp.kt` passes `appViewModel.scannerProcessor` to both call sites. `AppViewModel` exposes `val scannerProcessor: ScannerProcessor` via Hilt constructor injection. Build confirmed successful.
- **Round 9:** Task 9 approved (2026-07-10). All four test files created at correct path (`src/app/src/test/java/de/curlybracket/grocery/`). `ExampleUnitTest.kt` deleted. Test dependencies added via version catalog: `coroutines-test` 1.11.0, `turbine` 1.2.0, `mockk` 1.13.16 — referenced as `libs.*` in `build.gradle.kts`. Test counts: ScannerProcessorTest (10), BarcodeAnalyzerThrottleTest (7), HouseholdIdExtTest (5), GroceryRepositoryImplTest (8) — 30 total, all ≥4 per file. All async tests use `runTest {}`. Flow assertions use Turbine `test { awaitItem() }` or `first()`. Mock setup is clean — `connector` in GroceryRepositoryImplTest is a required constructor dependency, not redundant. Scope fix verified: `GroceryRepositoryImpl` promoted to `@Singleton` (class + module binding). All tests pass.
