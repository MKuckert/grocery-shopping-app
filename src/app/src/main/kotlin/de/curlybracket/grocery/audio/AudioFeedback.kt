 ...
 * SoundPool loads audio asynchronously; play calls are guarded with loaded flags.
...
class AudioFeedback @Inject constructor(
    ...
    fun playSuccess() {
        if (successLoaded) soundPool.play(successId, 1f, 1f, 1, 0, 1f)
    }

    fun playFailure() {
        if (failureLoaded) soundPool.play(failureId, 0.7f, 0.7f, 1, 0, 0.8f)
    }
